...
- **Never break the compiled contract.** `get_compiled()`, `compiled_pair()`, `compiled_pair_nd()` must return callables that produce identical results to eager paths. Test both.
